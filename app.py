...
with tab1:
    ...
    st.markdown(
        "Scans orders through the XGBoost delay predictor and "
    ...
    if model_artifact is None:
        ...
        if scan_btn or "high_risk_df" in st.session_state:
            ...
            if hr_df.empty:
                st.warning("⚠ No orders found. Run 01_setup_database.py first.")
            else:
                ...
                if is_fallback:
                    ...
                    st.error(f"🚨  **{len(hr_df)}** high-risk orders exceed **{risk_threshold:.0%}** threshold!")
                ...
                if prod_col:
                    ...
                    if st.button("🔍 Get Alternate Vendors", type="secondary"):
                        ...
                        if vendors is None or vendors.empty:
                            ...
                            # Use .map() — works on pandas >= 2.1
                            # Falls back to plain dataframe if styling fails
                            try:
                                styled = vendors.style.map(
                                    colour_score, subset=["Composite_Score"]
                                ).format({"Base_Price_INR": "₹{:,.0f}",
                                          "Composite_Score": "{:.3f}"})
                                st.dataframe(styled, use_container_width=True)
                            except Exception:
                                vendors["Composite_Score"] = vendors["Composite_Score"].round(3)
                                vendors["Base_Price_INR"]  = vendors["Base_Price_INR"].apply(
                                    lambda x: f"₹{x:,.0f}")
        ...
        if run_ghost or "ghost_result" in st.session_state:
            ...
            if ghost_df.empty:
                ...
                g2.metric("🚨 Ghost POs",     f"{len(ghosts):,}",
                          delta=f"{len(ghosts)/max(len(ghost_df),1):.1%}")
                ...
                top_ghosts = ghosts[disp_cols].head(50).copy() if disp_cols else ghosts.head(50).copy()
                ...
                if "Days_Since_Order" in ghost_df.columns:
                    val_col = next((c for c in ["Sales","Order_Item_Total"]
                                   if c in ghost_df.columns), None)
                    if val_col:
                        sample_df = ghost_df.sample(min(1000, len(ghost_df)))
                        fig_sc = px.scatter(
                            sample_df,
                            x="Days_Since_Order", y=val_col,
                            color=sample_df["Is_Ghost_PO"].map({0:"Normal",1:"Ghost PO"}),
                ...
                st.warning(
                    f"💡 Bulk-cancelling **{len(ghosts)}** Ghost POs "
        ...
        if df_sample.empty:
            ...
            if ship_col and late_col:
                ...
                fig_mode = px.bar(ship_late, x="Shipping_Mode", y="Late_Rate",
                                  ...
                                  title="Late Delivery % by Shipping Mode (label=order count)",
                                  template="plotly_dark")
                fig_mode.update_traces(texttemplate="%{text} orders", textposition="outside")
